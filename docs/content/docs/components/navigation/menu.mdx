...
<Demo preview="Scenes/Navigation/Menu.loom.tsx" previewHeight={450} screenshot="/images/screenshots/Navigation/Menu.png">
```tsx
...
`Menu.Item`'s prop type extends `ButtonProps`, but only the props below are currently forwarded to the underlying `Button`.
...
The underlying button is always rendered with `group` enabled, so a `Menu.Item` always participates in the menu's shared sizing group — this is not configurable per item.

Other `ButtonProps` fields (such as `intent`, `spacing`, `group`, or `disabled`) type-check because `Menu.Item`'s prop type extends `ButtonProps`, but they are not currently forwarded to the underlying `Button` and have no effect.
