...
`spinning` icons also forward every prop, including native `ImageLabel` properties, to the underlying `Icon`.
...
| `BackgroundTransparency`  | `number`                    | `1`                         | Overrides the icon's background transparency.                                         |

All other compatible native `ImageLabel` properties, such as `ZIndex`, `Visible`, `Position`, `ImageTransparency`, `ScaleType`, `Change`, and `Event`, are also accepted.
...
<Demo preview="Scenes/Surface/AllIcons.loom.tsx" previewHeight={460} screenshot="/images/screenshots/Surface/AllIcons.png">
```tsx
...
* Native `ImageLabel` properties, including `Change` and `Event`, are forwarded to the underlying instance whether or not `spinning` is enabled.
