...
## Native Frame Properties

`Select` renders its root as a [`Container`](/components/layout/container), so it extends `React.InstanceProps<Frame>` and accepts native Roblox `Frame` properties directly, rather than the `TextBox` properties its underlying control might suggest.
...
See the `Container` documentation for the full set of native `Frame` properties `Select` accepts, such as `AnchorPoint`, `LayoutOrder`, `Visible`, `Change`, and `Event`.
...
| Property   | Type                              | Default | Description                                                                    |
| ---------- | ---------------------------------- | ------- | -------------------------------------------------------------------------------- |
| `Event`    | `React.InstanceEvent<Frame>`       | —       | Native Roblox events for the underlying `Frame`.                                |
| `Change`   | `React.InstanceChangeEvent<Frame>` | —       | Native property-changed handlers for the underlying `Frame`.                    |

All other compatible native `Frame` properties inherited from `React.InstanceProps<Frame>` are also accepted—see [Native Frame Properties](#native-frame-properties) above.
...
The select's appearance is controlled by `theme.components.select`. Its `backgroundImage` value supplies the closed control's background image when the `backgroundImage` prop is omitted. Existing select theme values continue to control typography, spacing, borders, corner radius, dropdown height, dropdown colour, and option intent colours.
