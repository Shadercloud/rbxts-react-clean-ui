...
`theme.components.increment` only exposes a `button` override, which is forwarded as `styleOverride` to the minus/plus `Button`s. Otherwise, `Increment`'s appearance is driven by:

* `theme.components.increment.button` — overrides theme values (colours, corners, border, etc.) for the minus/plus `Button`s only.
* `theme.components.button` — used by the minus and plus `Button`s when no `theme.components.increment.button` override is set.
