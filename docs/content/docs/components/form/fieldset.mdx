...
<Demo preview="Scenes/Form/Fieldset.loom.tsx" previewHeight={320} screenshot="/images/screenshots/Input/Fieldset.png">
```tsx
<Fieldset>
    <Fieldset.Label>
...
When the parent `Fieldset` has `checkbox` set, `Fieldset.Control` instead keeps its own content size (matching `Fieldset.Label`'s behaviour), which is what `Checkbox` relies on to sit naturally next to its label. See [`Checkbox`](/components/form/checkbox) for an example.
...
| `checkbox`    | `boolean`                 | `false`     | Marks the fieldset as pairing a checkbox with its label, so `Fieldset.Control` sizes to its content instead of growing/shrinking. |
