...
`Container`-only props such as `group` and `backgroundImage` are also inherited and forwarded unchanged — see [Container](/components/layout/container) for details.
