...
<Demo preview="Scenes/Grid.loom.tsx" previewHeight={300} screenshot="/images/rowcolumn_preview.gif" screenshotAlt="Responsive Row and Column grid demonstration">
```tsx
...
Use the `spacing` prop on `Row` to control the gutter between columns. It accepts a theme spacing scale key (`"xs"`, `"sm"`, `"md"`, `"lg"`, `"xl"`) or `"None"`, and falls back to the theme's default spacing when omitted.
