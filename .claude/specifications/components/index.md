...
- `intent` (one of `primary`, `secondary`, `success`, `warning`, `danger`, `info`) is the shared semantic-color knob for interactive/status components. It selects a themed color scheme rather than the component taking raw color props as its primary styling surface.
