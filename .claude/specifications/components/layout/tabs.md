...
- `Tabs.Title` accepts a required `text: string` prop and defines a tab's button label; it renders nothing itself (children on `Tabs.Title`, if any, are not used).
...
- Tab button content (icon/text-equivalent) is text-only: each button renders its `Tabs.Title.text` through the shared `Text` component, styled from `theme.components.tabs.button` (corner radius, padding, shadow, text color, and typography), auto-sized (`Size={UDim2.fromScale(0,0)}`, `AutomaticSize.XY`).
...
- `spacing` (top-level) controls the content panel's padding.
- `list.backgroundColor`, `list.cornerRadius`, and `list.spacing` style the tab-button row container. `list.borderColor`/`list.borderThickness` are declared but not read in `Tabs.tsx`.
- `button.cornerRadius` and `button.spacing` style each tab button's corner rounding and internal padding; `button.boxShadow` supplies the shadow via the resolved intent scheme; `button.typography` styles the tab label text; `button.intents` supplies the `default`/`hover`/`focus` color schemes resolved through the shared intent-color system against the `"primary"` intent.
- `button.borderThickness` is declared but not read in `Tabs.tsx`.
- Component props take precedence over theme defaults only where `Tabs` exposes an override — currently `Tabs` exposes no direct style-override props beyond `scale`, so all `theme.components.tabs` values apply unconditionally.
