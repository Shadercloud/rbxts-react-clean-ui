...
- `Tabs` accepts an optional `backgroundImage` prop (a `CssBackgroundImage`) that overrides `theme.components.tabs.backgroundImage` for the content panel background, resolved the same way as `Select`'s `backgroundImage` prop (`props.backgroundImage ?? theme.components.tabs.backgroundImage`, passed straight through to the content panel's `Container`).
...
- The content panel is a `Container` with an inner border (`Enum.BorderStrokePosition.Inner`, styled from `theme.components.tabs.borderColor`/`borderThickness`), padding from `theme.components.tabs.spacing`, the same corner radius as the tab list (`theme.components.tabs.list.cornerRadius`) — reused rather than a separate content corner radius — and an optional background image resolved from `theme.components.tabs.backgroundImage`/the `Tabs` `backgroundImage` prop (see Public API and Theme).
...
- `backgroundImage` (a `CssBackgroundImage`, resolved the same way as `Box`/`Select`'s `backgroundImage`) is read directly and rendered on the content panel's `Container` — it does not affect `BackgroundColor3`/`BackgroundTransparency` on that `Container` (neither is set there), so the image renders regardless of the (unused) `backgroundColor` field above. The `Tabs` component's own `backgroundImage` prop takes precedence over this theme value.
...
- Component props take precedence over theme defaults only where `Tabs`/`Tabs.Title` expose an override — beyond `scale` and `backgroundImage` on `Tabs` and the padding props on `Tabs.Title`, no other direct style-override props exist, so the remaining `theme.components.tabs` values apply unconditionally.
